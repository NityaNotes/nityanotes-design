    ...
    return html`
      <div class="flex min-h-[100svh] items-center justify-center bg-[var(--background-color)] px-6 py-10 text-[var(--foreground-color)]">
        <div class="w-full max-w-2xl rounded-[var(--layout-radius-lg)] border border-[var(--border-color)] bg-[var(--surface-color)] p-8 shadow-sm sm:p-12">
          <p class="font-mono text-xs uppercase tracking-[0.24em] text-[var(--primary-color)]">${errorContent.eyebrow}</p>
          ...
          <div class="mt-8 flex flex-wrap items-center gap-3">
            ...
            >
              ${errorContent.homeLabel}
            </a>
            <span class="text-sm text-[var(--muted-color)]">${errorContent.routeLabel} <span class="font-mono text-[var(--foreground-color)]">${errorContent.route}</span></span>
          </div>
        </div>
      </div>
    `;
